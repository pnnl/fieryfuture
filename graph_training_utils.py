...
class ModelRunner:
    ...
    def train_step(self, datadict, eval_tensors=[], update=True):
        ...
        return self.sess.run(train_op + eval_tensors,
                             feed_dict=get_feed_dict(datadict, self.ph_dict, train=int(update), debug=self.debug))
    ...
    for k, v in ph_dict.iteritems():
        if type(v) is not list:
            ...
            for tensor, matrix in zip(v, datadict[k]):
                fd[tensor] = matrix
    dropout_placeholders = tf.get_collection('dropout_prob_placeholder')
    dropout_values = tf.get_collection('dropout_prob_value')
    bn_deciders = tf.get_collection('bn_deciders')
    for idx, prob in enumerate(dropout_placeholders):
        if train == 1:
            fd[prob] = dropout_values[idx]
        else:
            fd[prob] = 1.0
    if bn_deciders:
